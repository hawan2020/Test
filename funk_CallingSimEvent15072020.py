...
def funk_CallingSimEvent12072020(CompType = 1, CMleft = 520, CMmode=720, CMright = 920, PMleft=96,PMmode=120, PMright=168, WeibullShape = 1.5,
                                 WeibullScale = 21900, SimPeriod = 87600,  PMinterval = 8760*2, 
                                 PMtolerance = 720, NumSimulations = 1000):


    from SimEventLib12072020 import SimEvent as SimEvent
    import numpy as np
    import pandas as pd
    ...
    SimMatrix = np.array(SimEvent(CompType, CMleft,CMmode, CMright, PMleft,PMmode, PMright, WeibullShape, WeibullScale, SimPeriod, PMinterval, PMtolerance))
    
    SimNum = np.full((SimMatrix.shape[0],1),1)
    SimMatrix = np.append(SimMatrix, SimNum, axis = 1)
    
    for i in range(0, NumSimulations - 1):
        Simulation = SimEvent(CompType, CMleft,CMmode, CMright, PMleft,PMmode, PMright, WeibullShape, 
